...
'''
3.2 实验中的模型类别
CNN-rand: 句子中的的word vector（即Embedding层的 W 参数初始值）都是随机初始化的，同时当做CNN训练过程中需要优化的参数；
...
CNN-non-static: 句子中的word vector（即Embedding层的 W 参数初始值）是使用word2vec预先对Google News dataset (about 100 billion words)进行训练好的词向量表中的词向量。在CNN训练过程中作为固定的输入，做为CNN训练过程中需要优化的参数；
'''
...
class TextCNN():
    ...
    __static_falg = True  # word vector（即Embedding层的 W 参数初始值）不作为优化的参数
    ...
    def __init__(self,W_list,shuffer_falg, static_falg, filter_numbers, filter_sizes,sentence_length,embedding_size,learnrate, epochs, batch_size, dropout_pro):
        ...
        with self.train_graph.as_default():
            ...
            with tf.name_scope('embedding_layer'):
                train_bool = not self.__static_falg#代表是否作为优化的参数
                # tf.convert_to_tensor(W_list,dtype=tf.float32)
                # pdb.set_trace()
                self.embedding_layer_W = tf.Variable(initial_value=W_list,dtype=tf.float32, trainable=train_bool, name='embedding_layer_W')#W_list有两种初始化方式，随机初始化/ Google news embedding
                print("ssssssss")#
                self.embedding_layer_layer = tf.nn.embedding_lookup(self.embedding_layer_W, self.input_x)#获取到W矩阵中input_x索引对应的那些行，组合形成一个矩阵并返回。即得到了batch个句子的向量化表示
                self.embedding_layer_expand = tf.expand_dims(self.embedding_layer_layer, -1)#增加维度，0表示在最前加1维，-1表示在最后加1维，主要是因为卷积层的输入的维度比此时多一维，所以要扩展一个维度
            ...
            with tf.name_scope('dropout_layer'):
                ...
                h_pool = tf.concat(pool_layer_lst,name='last_pool_layer',axis=3)#连接在一起形成一个向量
            ...
            with tf.name_scope('soft_max_layer'):
                ...
                # print('test2------------')
                self.l2_loss += tf.nn.l2_loss(SoftMax_W)#正则化项采用L2,作用在softmax层上，因为softmax是全连接，所以可能会过拟合，故而正则化  softmax_w和softmax_b
                ...
                self.predictions = tf.argmax(self.softmax_values,axis=1,name='predictions',output_type=tf.int32)#输出比较大的那个，即为预测的分类

            with tf.name_scope('loss'):
                losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.softmax_values,labels=self.input_y)#交叉熵，此处为两个二维的向量求交叉熵
